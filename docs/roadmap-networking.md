...
### N9.1 — Realtek RTL8139
- 最经典的百兆网卡，PCI设备
- 极简设计，适合学习
- TX/RX描述符环，中断处理
- PCI vendor 10EC, device 8139
- **~300行**

### N9.2 — Realtek RTL8169/8111
- 最常见的千兆网卡（绝大多数PC主板）
- PCI/PCIe设备，DMA描述符
- TX/RX环形描述符（256个）
- Checksum offload, VLAN tag offload
- PCI vendor 10EC, device 8168/8169
- **~500行**

### N9.3 — Intel I210/I211
- 中端桌面千兆网卡
- 寄存器布局接近e1000e，可复用部分代码
- MSI-X中断，多队列
- PCI vendor 8086, device 1533/1539
- **~400行**

### N9.4 — Intel I225-V (2.5GbE)
- 新一代桌面网卡（2.5 Gbps）
- Z490/B550及以后主板标配
- PCI vendor 8086, device 15F3
- **~450行**

### N9.5 — Intel X710/XL710 (i40e, 10GbE)
- 服务器级万兆网卡
- Admin Queue命令接口
- 多队列（最多1536个）
- RSS, DCB, SR-IOV
- PCI vendor 8086, device 1572/1583
- **~1000行**

### N9.6 — Intel E810 (ice, 25/100GbE)
- 最新服务器网卡
- 灵活的包分类管道
- PTP硬件时间戳
- 100Gbps带宽
...
### N9.7 — Mellanox ConnectX-5/6 (mlx5, 25/100/200GbE)
- 数据中心标准网卡
- RDMA/RoCE支持
- 硬件流表（Flow Steering）
- 200Gbps带宽
...
### N9.9 — USB网卡
- ASIX AX88179 (USB 3.0千兆)
- Realtek RTL8153 (USB 3.0千兆)
- CDC-ECM已有基础
- **~400行**

### N9.10 — WiFi网卡
- Intel WiFi 6 (iwlwifi/AX200) 框架
- Realtek RTL8821CE
- 802.11ax (WiFi 6) 基础帧处理
- **~1500行**
...
| 网卡 | 速率 | 场景 | 优先级 | 预估 |
|------|------|------|--------|------|
| e1000/e1000e | 1G | QEMU/老PC | ✅ 已完成 | — |
| virtio-net | 10G+ | 虚拟化 | ✅ 已完成 | — |
| RTL8139 | 100M | 老PC | ⭐⭐ | 300行 |
| **RTL8169/8111** | 1G | **大多数PC** | ⭐⭐⭐ | 500行 |
| I210/I211 | 1G | 中端桌面 | ⭐⭐ | 400行 |
| **I225-V** | 2.5G | **新主板** | ⭐⭐⭐ | 450行 |
| X710 (i40e) | 10G | 服务器 | ⭐⭐ | 1000行 |
| E810 (ice) | 100G | 数据中心 | ⭐ | 1500行 |
| mlx5 | 200G | 数据中心 | ⭐ | 2000行 |
| BCM57xx | 10G | 服务器OEM | ⭐ | 800行 |
| AX88179/RTL8153 | 1G | USB网卡 | ⭐⭐ | 400行 |
| iwlwifi/RTL8821 | WiFi 6 | 笔记本 | ⭐⭐ | 1500行 |
...
N9 网卡驱动:   +8,850 行 → 25,050
```

网络子系统总计: **~25,050 行**（占内核总代码 ~20%）
